<template>
  <section class="mx-auto w-full max-w-[69rem] space-y-4">
    <p class="m-0 text-sm text-muted">Pick a month to filter ETF positions by open date.</p>

    <FMonthCalendar v-model="selectedPeriodMonth" />

    <section class="rounded-[12px] border border-border bg-surface p-4">
      <div class="mb-3 flex flex-wrap items-center justify-between gap-3">
        <h2 class="m-0 text-lg font-bold text-text">{{ selectedMonthLabel }} - ETFs</h2>
      ...
      <p v-if="loading" class="m-0 text-sm text-muted">Loading ETFs...</p>
...
const getFxDateForRow = (row: EditableEtfRow) => row.closedAt || new Date().toISOString().slice(0, 10)

const rowProfitLossRaw = (row: EditableEtfRow) => (row.currentPrice - row.openPrice) * row.quantity

const rowProfitLossDisplay = (row: EditableEtfRow) => {
...
const loadRows = async () => {
  ...
  try {
    ...
    editableRows.value = ((data ?? []) as EtfRow[]).map((row) => {
      const editableRow: EditableEtfRow = {
        localId: row.id,
        id: row.id,
        name: row.name,
        currency: row.currency,
    ...
    errorMsg.value = error instanceof Error ? error.message : 'Failed to load ETF rows.'
  } finally {
    loading.value = false
  }
}
...
const saveRow = async (localId: string) => {
  ...
  try {
    if (row.id) {
      const { error } = await supabase.from('etfs_positions').update(payload).eq('id', row.id)
      if (error) throw error
    } else {
      const { data, error } = await supabase.from('etfs_positions').insert(payload).select('id').single()
      if (error) throw error
      row.id = data.id
      row.localId = data.id
    }
...
const deleteRow = async (localId: string) => {
  ...
  try {
    if (row.id) {
      const { error } = await supabase.from('etfs_positions').delete().eq('id', row.id)
      if (error) throw error
    }
